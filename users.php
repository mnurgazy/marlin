    ...
    <body class="mod-bg-1 mod-nav-link">
        ...
        <main id="js-page-content" role="main" class="page-content mt-3">
            ...
            <div class="row">
                ...
                <?php echo ($_SESSION['role'] == 'admin') ? '<a class="btn btn-success" href="create_user.php">Добавить</a>' : ""?>
                            ...
                            <div class="d-flex flex-row align-items-center">
                                <?php 
                                    $statusList = array (
                                                ...
                                                'away' => 'status-warning',
                                                'dont_disturb' => 'status-danger'
                                            );
                                ?>                                
                                <span <?php
                                    foreach ($statusList as $status => $value) {
                                        if ($user['status'] == $status) 
                                            echo 'class="status '.$value.' mr-3"';
                                        elseif (empty($user['status']))
                                            echo 'class="status status-dark mr-3"';
                                    }
                                    ?>>
                                <a href="<?php echo 'page_profile.php?user_id='.$user['user_id']; ?>">
                                    <?php if (!empty($user['avatar'])): ?>
                                    <span class="rounded-circle profile-image d-block " style="background-image:url('<?php echo $user['avatar']; ?>'); background-size: cover;">  
                                    </span>
                                    <?php else: ?>
                                    <span class="rounded-circle profile-image d-block " style="background-image:url('img/demo/avatars/avatar-f.png'); background-size: cover;">  
                                    </span>
                                </a>
                                    <?php endif; ?>
                                ...
                                <div class="info-card-text flex-1">
                                        <a href="javascript:void(0);" class="fs-xl text-truncate text-truncate-lg text-info" data-toggle="dropdown" aria-expanded="false"><?php echo $user['name']; ?></a>

                                        <?php if ($_SESSION['role'] == 'admin' || ($_SESSION['log-in']) == $user['email']) : ?>
                                    ...
                                    <div class="dropdown-menu">
                                        <a class="dropdown-item" href="<?php echo 'edit.php?user_id='.$user['user_id'];?>">
                                            <i class="fa fa-edit"></i>
                                        Редактировать</a>
                                        <a class="dropdown-item" href="security.php?user_id=<?php echo $user['user_id'];?>">
                                            <i class="fa fa-lock"></i>
                                        Безопасность</a>
                                        <a class="dropdown-item" href="status.php?user_id=<?php echo $user['user_id'];?>">
                                            <i class="fa fa-sun"></i>
                                        Установить статус</a>
                                        <a class="dropdown-item" href="media.php?user_id=<?php echo $user['user_id'];?>">
                                            <i class="fa fa-camera"></i>
                                            Загрузить аватар
                                        </a>
                                        <a href="delete_form.php?user_id=<?php echo $user['user_id'];?>" class="dropdown-item" onclick="return confirm('Вы действительно хотите удалить?');">
